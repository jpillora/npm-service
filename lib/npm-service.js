...
mkdirp.sync(TMP_DIR);
//disk cache dir
var CACHE_DIR = path.join(TMP_DIR, 'cache');
...
//inmemory 150MB cache
var cache = require("lru-cache")({
  max: 150*1024*1024,
  length: function (buff) {
    return buff.length;
  },
  //keep files cache also under 'max'
  dispose: function(key) {
    fs.unlink(path.join(CACHE_DIR, key), function() {});
  }
});

//preload into memory (sync)
var files = fs.readdirSync(CACHE_DIR);
files.forEach(function(f) {
  cache.set(f, fs.readFileSync(path.join(CACHE_DIR, f)));
});
...
exports.install = function(deps, opts, callback) {
  ...
  function finalize() {
    ...
    src.on('data',function(buff) {
      ...
      //write to disk
      fs.writeFile(path.join(CACHE_DIR, depsHash), buff);
      if(callback)
        callback(null, buff);
    });
  }
